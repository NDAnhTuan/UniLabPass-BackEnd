...
public interface ModelClient {
    @PostMapping(value = "/model/verify", consumes = MediaType.MULTIPART_FORM_DATA_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseEntity<String> verify(@RequestPart("image1") MultipartFile image1,
                                  @RequestPart("image2") MultipartFile image2);
    @GetMapping(value = "/model/healthcheck", produces = MediaType.APPLICATION_JSON_VALUE)
    String healthcheck();
}
